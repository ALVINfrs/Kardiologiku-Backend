...
export class Medication {
    ...
    static async findByUserId(userId: number) {
        const query = 'SELECT * FROM medications WHERE user_id = ? ORDER BY name ASC';
     ...
     */
    static async update(medId: number, userId: number, medData: Partial<IMedication>) {
    ...
    static async logTaken(userId: number, medicationId: number) {
        ...
        const decrementStockQuery = 'UPDATE medications SET stock = stock - 1 WHERE id = ? AND user_id = ? AND stock > 0';
        await db.query(decrementStockQuery, [medicationId, userId]);
        
        // Catat ke dalam log
        const logQuery = 'INSERT INTO medication_log (user_id, medication_id) VALUES (?, ?)';
